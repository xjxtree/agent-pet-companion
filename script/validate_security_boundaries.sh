...
assert_json "$INGEST" 'data["event"]["title"] == "执行工具" and data["event"]["detail"] is None and set(data["event"]["payload_json"]) == {"schema_version", "contract_version", "external_event_id", "source_event", "tool_name", "outcome", "diagnostic", "affects_activity", "turn_id", "session_active", "message_role", "message_content", "activity_kind", "activity_content", "interaction_kind", "project_label", "session_title", "session_open", "session_surface", "terminal_app", "session_open_url"} and data["event"]["payload_json"]["external_event_id"] == "evt_safe_alias_normalization" and data["event"]["payload_json"]["source_event"] == "unclassified" and data["event"]["payload_json"]["tool_name"] == "other" and data["event"]["payload_json"]["outcome"] == "unknown" and data["event"]["payload_json"]["contract_version"] is None and data["event"]["payload_json"]["affects_activity"] is True'
...
assert_json "$RECENT" 'data[0]["id"] == "evt_safe_alias_normalization" and data[0]["title"] == "执行工具" and data[0]["detail"] is None and set(data[0]["payload_json"]) == {"schema_version", "contract_version", "external_event_id", "source_event", "tool_name", "outcome", "diagnostic", "affects_activity", "turn_id", "session_active", "message_role", "message_content", "activity_kind", "activity_content", "interaction_kind", "project_label", "session_title", "session_open", "session_surface", "terminal_app", "session_open_url"} and data[0]["payload_json"]["contract_version"] is None and data[0]["payload_json"]["affects_activity"] is True'

SNAPSHOT="$(APC_HOME="$TMP_DIR/home" "$ROOT_DIR/target/debug/petcore-cli" snapshot 2>&1)"
assert_no_secret "state snapshot redaction" "$SNAPSHOT"
assert_no_raw_alias "state snapshot normalization" "$SNAPSHOT"
assert_json "$SNAPSHOT" 'len(data.get("events", [])) >= 1 and len(data.get("recent_events", [])) >= 1 and all(set(item) == {"id", "source", "session_id", "event_type", "title", "created_at"} and item["id"].startswith("evt-") and len(item["id"]) == 68 and item["id"] != "evt_safe_alias_normalization" and item["title"] == "执行工具" for item in data.get("events", []) + data.get("recent_events", []))'
