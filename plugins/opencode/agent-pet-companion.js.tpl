...
function boundedActivity(value, policy = "safe") {
  if (value == null) return undefined;
  const scalar = normalizeActivityNode(value, 0, { nodes: 256 }, new WeakSet(), policy, 0);
...
function activityDetail(properties) {
  const candidates = [
    [properties?.activity_content, properties?.activity_kind === "command" ? "command" : "safe"],
    [properties?.reasoning_summary, "safe"],
    [properties?.reasoning, "safe"],
    [properties?.summary, "safe"],
    [properties?.detail, "safe"],
    [properties?.command, "command"],
    [properties?.arguments, "safe"],
    [properties?.input, "safe"],
    [properties?.result, "safe"],
    [properties?.output, "safe"],
    [properties?.content, "safe"],
    [properties?.error?.message, "safe"],
    [properties?.error, "safe"],
  ];
  const candidate = candidates.find(([value]) => value != null);
  return candidate == null ? undefined : boundedActivity(candidate[0], candidate[1]);
}
