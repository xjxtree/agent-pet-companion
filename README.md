...
The provider-neutral [`agent-pet-maker`](skills/agent-pet-maker/SKILL.md) skill is bundled with development app packages and can be used by Codex, Claude Code, Pi, Hermes, OpenCode, or another Agent Skills host that has real image understanding and image generation/editing. It supports `create` and `modify`, uses `petcore-cli` as the final validator/builder, preserves IDs during modification, verifies unchanged state hashes, and returns `capability_missing` rather than fabricating images when the host lacks a real image tool. Creating or finalizing a package never changes the library. The separate online `install` command is available only when the user explicitly asks to import it; activation requires a second explicit `--activate` choice and never enables global desktop-pet behavior.

Copy the **entire** `agent-pet-maker` directory, including `references/`, `scripts/`, `agents/`, and `tests/`; copying only `SKILL.md` is not sufficient. In this repository it lives at `skills/agent-pet-maker/`; in a built app it lives under `AgentPetCompanion.app/Contents/Resources/skills/agent-pet-maker/`. Current host locations are:

| Host | Installation location | Invocation |
|---|---|---|
| [Codex](https://developers.openai.com/codex/skills/) | In the App, open **Agent Connections → Codex → Repair**. The managed plugin installs the complete skill under `~/.agents/plugins/plugins/agent-pet-companion/skills/agent-pet-maker/`. | Ask naturally or explicitly mention the reliable plugin-qualified name `$agent-pet-companion:agent-pet-maker`. Codex normally detects the repaired plugin automatically; restart Codex if an existing session does not show the skill. The short `$agent-pet-maker` alias resolves but does not inject the skill body in Codex CLI 0.144.4. |
