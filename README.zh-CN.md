...
- App 内支持低分辨率和标准分辨率制作。
- 默认使用 ChatGPT/Codex 内置生图生成原生透明 PNG 来源图；导入前仍会由共享制作流水线核验实际 Alpha、来源尺寸、边缘、动作和最终 `.petpack`。
- 制作任务会保留进度；遇到需要确认的问题时，可以直接在 App 内回复。
- **通用宠物制作技能**按钮可在 `~/agent/skills/agent-pet-maker` 安装和管理通用 `agent-pet-maker` 工作流。App 会展示依赖与版本，是否能发现该目录则由用户根据 Agent 自行确认。
