...
- **AI宠物制作**：支持高自由度、任意风格的宠物制作，可选择标清 192×208 或标准 384×416 原生画质；标准画质默认适合大多数角色，标清适合极简或像素风格。每个有效帧都必须保留所选画质的原生来源分辨率且禁止低分辨率放大，每个状态独立声明逐帧时长、播放方式与减弱动态效果代表帧。
- **多 Agent 会话支持**：按 Agent 汇总 Codex、Claude Code、Pi Coding Agent 和 OpenCode 在所有项目中的会话，并区分其 App 与 CLI 来源；折叠气泡显示一个优先提醒或最新会话，展开后直接显示本地有界快照中的全部具体会话。每行右侧 badge 会标明与宠物动作一致的具体状态，并显示 App 或 CLI 标签；标题下两行专用于展示有界 Agent 消息、宿主提供的 reasoning、命令、工具输入输出、错误或原始活动详情。通过校验的路由可返回 ChatGPT/Codex 或 Claude App 的精确会话、Warp CLI 会话，或相应 App/终端宿主；不可用的路由不会提供误导性点击。
- **本地优先**：宠物、设置、有界会话上下文与诊断信息都保留在 Mac 上，只有你主动导出时才会生成外部文件；AI 宠物制作仅在你主动开始创建或修改时使用已配置的 Codex 服务。

## 功能

- **宠物库**：使用内置的 `星雾团子` 与 `Bytebud 字节芽`，或导入、预览、启用、导出和管理自己的 `.petpack` 宠物；本地预览损坏时可从不可变宠物包重新校验并恢复。
...
- **桌面悬浮层**：宠物本体在启动和状态切换期间也始终可拖动，气泡与菜单会作为同一组合一起移动，并可从原生气泡打开活动会话。显示尺寸只在“宠物配置”中调整，因此拖动始终只负责移动桌宠。
...
只有在切换或导入宠物、创建或修改宠物、调整桌宠体验、连接 Agent，或恢复服务与导出诊断时，才需要打开五个管理页面。AI 制作需要可用的 Codex App Server 与当前用户已配置的服务访问权限。运行时会直接播放 V2 各状态声明的逐帧制作时序，不提供播放速度档位，不抽帧，也不会在卡顿后快速补播错过的帧。
...
| [`.petpack` V2 规范](docs/specifications/AgentPetCompanion_Petpack_Whitepaper_V2.md) | 可移植宠物格式、制作时序与生产者契约 |
