...
- **AI宠物制作**：支持高自由度、任意风格的宠物制作，可选择高分辨率宠物画质；已有宠物也支持通过 AI 修改，并通过身份锁定、动作导演与真实速率视觉复核保持非运动区域、锚点、道具和动作节奏连贯。
- **多 Agent 会话支持**：按 Agent 汇总 Codex、Claude Code、Pi Coding Agent 和 OpenCode 在所有项目中的会话；折叠气泡显示一个会话，展开后最多显示三个，更多会话进入 Agent 连接页，整行可在可用时打开相应宿主或精确会话。
...
- **Agent 连接**：分别显示本地集成健康度与真实任务验证，并为 Codex、Claude Code、Pi Coding Agent 和 OpenCode 提供类型化检查、一键设置或修复、测试和受管移除操作。
