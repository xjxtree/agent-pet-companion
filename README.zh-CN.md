...
- **多 Agent 会话支持**：按 Agent 汇总 Codex、Claude Code、Pi Coding Agent 和 OpenCode 在所有项目中的会话；折叠气泡显示一个优先提醒或最新会话，展开后直接显示本地有界快照中的全部具体会话。每行会标明与宠物动作一致的具体状态，同时显示安全的当前活动摘要与最新 Agent 消息，并可在可用时打开相应宿主或精确会话。
