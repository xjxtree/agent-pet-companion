...
- **AI宠物制作**：支持高自由度、任意风格的宠物制作，可选择高分辨率宠物画质；每个有效帧都必须保留所选画质的原生来源分辨率且禁止低分辨率放大，并在每张素材和每个状态完成后立即校检，避免到全部动画生成完毕才发现问题。
- **多 Agent 会话支持**：按 Agent 汇总 Codex、Claude Code、Pi Coding Agent 和 OpenCode 在所有项目中的会话；折叠气泡显示一个优先提醒或最新会话，展开后直接显示本地有界快照中的全部具体会话。每行右侧 badge 会标明与宠物动作一致的具体状态，标题下两行则用于展示有界 Agent 消息、宿主提供的 reasoning、命令、工具输入输出、错误或原始活动详情，并可在可用时打开相应宿主或精确会话。
