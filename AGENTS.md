...
- Render resolution has three exact runtime tiers, all 12:13 and independent of display size: `low` 192×208, `standard` 384×416 (default), and `high` 576×624. 192×208 is the hard floor and every width is a multiple of 192, which keeps sheet edges on 16-pixel boundaries. One package uses one tier for every frame. An image model is not required or trusted to return those exact dimensions: every newly generated frame starts from a complete 12:13 source crop at least as large as its selected target. ChatGPT/Codex built-in `imagegen` and the App's Codex-backed Studio request genuine native-Alpha PNG source art by default for `low` and `standard`; the shared transparent-frame script preserves the authored Alpha plane and nontransparent RGBA, clears only hidden RGB beneath Alpha 0, retains the source-resolution transparent master, and may perform one direct linear-light premultiplied-Alpha downscale. An explicitly selected opaque-only provider instead uses the script's bounded flat-chroma compatibility mode. Upscaling, super-resolution, padding a smaller crop to a target canvas, independent per-pose fit/recentering, cascaded or post-process resizing, ad hoc matte/edge filters, silently replacing a failed native-Alpha action with chroma, and substituting multiple batches for missing source capacity are invalid. Downscaling never relaxes runtime-size identity, Alpha/background integrity, action, distinct-pose, anatomy, prop, crop, continuity, or playback QA. Package/runtime support is broader than producer capability: external source-capable workflows may build `high`, while built-in `imagegen` and Studio must reject `high` before attempting generation.
