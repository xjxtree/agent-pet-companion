...
Adapters record the actual App or CLI origin. Audited App markers override inherited terminal variables for Codex and OpenCode; Claude requires live process corroboration because its App markers also reach nested CLI calls. Prompts, titles, project paths, and previous card order never infer origin. / 适配器记录实际 App 或 CLI 来源。Codex 和 OpenCode 使用经审核的 App 标记；Claude 的 App 环境标记会被嵌套 CLI 继承，因此必须由实时进程链佐证。提示词、标题、项目路径和卡片顺序均不用于猜测来源。
...
Rows expose `exact_session`, `agent_host`, or `unavailable`. After the declared route succeeds, the App acknowledges a completed row or persistently closes a failed row through `agent.session.dismiss`; either stays hidden after App restart. Opening a host after an exact deep-link failure is recovery assistance, not proof that the session opened, so the row remains visible. Unavailable, failed, and degraded navigation notices last three seconds, then reveal the latest retained session message; another attempt restarts that interval. A newer session event clears the old notice immediately. Rows without a successful declared route still require manual close. / 会话按声明的目标成功跳转后，已完成气泡保存完成确认，失败气泡通过 `agent.session.dismiss` 持久关闭，重启 App 后均不恢复。精确跳转失败后仅打开 Agent App 不算成功到达会话，气泡继续保留。无法跳转、打开失败和降级提示显示三秒后恢复会话最新消息，再次尝试重新计时；新的会话事件会立即清除旧提示。未成功到达声明目标的气泡仍需用户手动关闭。
...
Claude hooks query PetCore's `agent.claude_context` while the hook process is alive, before transcript recovery and ordinary event ingestion. PetCore walks at most 32 same-user process ancestors using only executable paths, parent PIDs and kernel start identities. It associates native `claude` / `claude.exe` executables (including version-named native installations) with hashed session identities in a 256-entry memory registry. A different session under an already registered, still-matching Claude process becomes the existing `session.child` marker; neither its reply nor its failure creates a bubble. An actual `Claude.app/Contents/MacOS/Claude` ancestor, before any additional Claude engine, corroborates the App surface. Environment markers alone never do. / Claude Hook 在自身进程仍存活时，先通过 `agent.claude_context` 查询 PetCore，再补读消息并写入普通事件。PetCore 最多检查 32 层同用户祖先进程，只读取可执行文件路径、父 PID 和内核启动标识，以最多 256 条内存记录关联原生 Claude 进程与会话标识的哈希。若不同会话位于已登记且启动标识仍匹配的 Claude 父进程下，则转换为现有 `session.child` 标记，回复和失败均不生成气泡。在遇到另一个 Claude 引擎之前发现真实 Claude App 主进程，才确认 App 来源；仅有环境标记不够。

Standalone CLI calls remain eligible even with `-p`, a short prompt such as `x`, a temporary directory, or an authentication error. Missing/reused/exited processes, an incomplete ancestry walk, unrecognized executables, multiplexed process ownership, or a parent not yet registered after PetCore restart never prove a child. Diagnostic probes and explicit sidechain markers do not register process ownership. The registry is transient: it cannot retrospectively classify already-exited historical calls. No process arguments, environment blocks, parent session IDs or process registry enter persistence or diagnostics; transcript `bridge-session` absence is not a classification rule. / 独立 CLI 即使使用 `-p`、短提示词 `x`、临时目录或发生认证错误，仍可正常显示。进程缺失、复用或退出，祖先链不完整，可执行文件无法识别，同一进程对应多个会话，或 PetCore 重启后父会话尚未登记时，都不会据此隐藏消息。诊断探针及显式 sidechain 标记不登记进程归属。关联仅保存在内存中，不能追溯识别已经退出的历史调用；不读取进程参数或完整环境，不持久化或记录父会话标识与进程关联表，也不以缺少 `bridge-session` 判定子会话。
...
Claude Desktop hooks omit the message-bearing prompt/stop events needed for a useful bubble. When a Claude hook supplies `transcript_path`, the adapter may read a fixed tail of that one file to recover only title, latest user prompt, and the latest visible Agent reply or thinking/progress summary. Nonempty string `thinking` blocks are eligible display copy; empty omitted thinking, encrypted `signature` fields, and `redacted_thinking` data are excluded. Newer summaries replace older replies, replies win within the same record, and recovery preserves the hook’s tool/activity status. The path must be absolute, current-user-owned, regular, and not a symlink; the read is size bounded. Tool results, structured message envelopes, sub-Agent turns, and transcript history are excluded. Failure leaves fields absent and never changes connector health.

Claude 的可见进度更新和思考摘要可能以非空 `thinking` 字符串保存；气泡按会话记录顺序选择最新可见内容，同一记录内优先正式回复。补读不改变工具活动状态，不解析或展示加密签名及隐藏思考内容，保留原有尾部读取、长度及根会话边界。会话记录没有提供已验证的 thinking display 模式，因此这里同时接受可见进度更新与思考摘要，不根据块序号将它们推断为仅含进度的消息。该格式依据 [Claude thinking display](https://platform.claude.com/docs/en/build-with-claude/thinking#progress-updates-between-tool-calls)。 / The visible-summary format follows the linked Claude thinking-display contract. The transcript does not establish a verified thinking display mode, so recovery accepts both visible progress updates and thinking summaries without inferring a progress-only classification from block positions.
...
| Claude Code | Tool descriptions, commands/results/errors, permission/input requests, sub-Agent and compaction hooks; explicit visible thinking/progress summaries from the hook-named transcript, never private model reasoning |
...
| DeepSeek Harness | Reasoning `block-start` boundaries, final-message visible text and explicit reasoning, clean tool names, single-line plan/todo projections, approval requests/decisions, and subagent lifecycle |
...
Ordinary `session_active` is a bounded observation, not a permanent heartbeat. `waiting` and `failed` remain until a newer event resolves them or the user explicitly closes the displayed message; a failed row also closes persistently after successful navigation as described above. Manual close is persisted for all sources and statuses as specified in the [data model](../architecture/data-model.md#agent-events-and-session-projection); it does not delete the Agent session or history. / 等待和失败状态持续显示，直到新事件解除状态或用户手动关闭当前消息；失败气泡成功跳转后也按上述规则持久关闭。所有来源、所有状态的手动关闭均按数据模型持久保存，不删除 Agent 会话或历史。 Repeated terminal notifications in one activity epoch share one completion acknowledgement identity; a later true activation creates a new scope.
...
DeepSeek Harness emits stream chunks and lifecycle broadcasts through Cordis. The managed plugin subscribes only to the four session-scoped broadcast `emit` events (`session/event`, `subagent/start`, `subagent/end`, and `session/disposed`); `agent/status` is audited but intentionally not subscribed because it has no session identity. It explicitly avoids waterfall/interception events (`agent/pre-step`, `tools/pre-execute`, etc.) to prevent pure observers from stalling the driver chain. `assistant/chunk` events are filtered so only explicit `block-start{blockType:'reasoning'}` boundaries emit `thinking`, while all delta chunks and usage data are dropped locally. At each final `assistant/message` boundary the plugin projects the assembled message at once: visible `text` blocks become bounded Agent body copy and explicit, non-redacted string `reasoning` blocks become bounded thinking body copy. This is a display-only edge that refreshes the bubble body during execution without restarting a thinking animation or pulling an in-progress tool state back to start/thinking; when one final message carries both, the visible text wins the same-sequence body selection. A text-less final step clears the cached assistant text so a later terminal edge cannot resurrect stale body over newer thinking. User messages arrive as a direct `user/message` payload (`event.data` is the `UserMessage`); only a real `source.kind === 'user'` counts as user context, with a bounded historical nested `{ message }` fallback. `todo/write` projects exactly one closed, user-readable plan line (first in-progress, else first pending, else last completed) and never serializes the whole list. Stable host sequence numbers become opaque event identities so repeated same-tool events do not collapse. Bounded user/assistant text and title caches are attached only to semantic start/terminal edges as a delivery fallback. Active child subagents tracked via `subagent/start`/`subagent/end` maintain a per-parent background fence: a parent `turn/end{completed}` while subagents are running emits `background_active`, and the terminal `done/completed` is deferred until all child runs in that parent's fence settle. Root `session/disposed` is an explicit close and immediately removes any waiting, completed, or failed bubble for that session.
