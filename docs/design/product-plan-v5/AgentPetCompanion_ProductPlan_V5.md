...
- 消息气泡整体使用 macOS 系统原生 Liquid Glass；macOS 26 采用最高透明度的 clear interactive glass，不加 tint、不透明填充、边框或玻璃层 opacity。气泡内容直接位于玻璃内容层；透明 `NSPanel` 不再额外使用会提升后代玻璃层级的共享容器，避免光学层遮住文字。旧系统使用原生 ultra-thin material 回退，不绘制不透明白色卡片。
