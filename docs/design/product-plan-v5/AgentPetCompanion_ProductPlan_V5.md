...
- 消息气泡整体使用 macOS 系统原生 Liquid Glass；macOS 26 以 AppKit clear glass 作为独立背景光学层，不加 tint、不透明填充或边框，并在“最大透明”视觉基线中仅以 18% 强度保留折射。文字、图标和控件作为 100% 不透明的 SwiftUI sibling 前景层，禁止再嵌入会在透明 `NSPanel` 中吞掉前景的 glass `contentView`，也不使用会提升后代玻璃层级的共享容器。旧系统使用原生 ultra-thin material 回退，不绘制不透明白色卡片。
