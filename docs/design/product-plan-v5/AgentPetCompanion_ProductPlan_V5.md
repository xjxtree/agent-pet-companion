...
主窗口采用 macOS 原生侧栏、统一工具栏与透明 Liquid Glass 视觉体系：macOS 26 使用 clear glass，macOS 14–15 使用系统 ultra-thin material 回退。外观主题提供「跟随系统」「黑色主题」「白色主题」三项，并同时作用于控制中心、状态栏菜单、桌宠消息气泡、收起/展开按钮、缩放按钮与原生右击菜单。页面只保留一层主要信息面板，状态与操作就近展示；主操作使用系统按钮，选中、禁用、高对比度和 Reduce Transparency 行为交给系统处理，不绘制模拟 macOS 的不透明卡片或菜单。
...
- 消息气泡整体使用 macOS 系统原生 Liquid Glass；macOS 26 以 AppKit clear glass 作为独立背景光学层，不加 tint、不透明填充或边框，光学强度随用户透明度设置变化且最高透明仍保留可辨识折射。主题只切换系统 appearance 与语义前景色，不改写气泡透明度，因此三种主题下保留同一透明度设置和光学强度。文字、图标和控件作为 100% 不透明的 SwiftUI sibling 前景层，不使用模糊、白色/黑色 halo 或文字阴影；禁止再嵌入会在透明 `NSPanel` 中吞掉前景的 glass `contentView`，也不使用会提升后代玻璃层级的共享容器。旧系统使用原生 ultra-thin material 回退，不绘制不透明白色卡片。
