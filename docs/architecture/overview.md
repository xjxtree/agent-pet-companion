...
For App creation and portable `low`/`standard` work, ChatGPT/Codex built-in image generation is the default image path and requests genuine native-Alpha PNG source art. Selected originals are copied from Codex-managed generated-image storage into the private job workspace, then the shared Maker script preserves decoded Alpha and visible RGBA, normalizes hidden transparent RGB, performs the sole optional downscale, and emits multi-background QA. A selected opaque-only provider uses the same script's explicit chroma compatibility mode; PetCore trusts neither mode until the common production gate passes.
