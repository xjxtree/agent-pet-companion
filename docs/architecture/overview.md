...
Cold launch and reopen use the registered control-center window as their single fronting authority. Initial desktop-pet presentation preserves an already-key control center, but never reactivates the App after the user has switched away; keyboard focus moves to the session bubble only through its explicit command. Closing the control center leaves the menu bar and enabled pet running. Standard Quit closes the UI host and overlay; the LaunchAgent-hosted PetCore may remain available.
