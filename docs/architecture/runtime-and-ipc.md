...
flowchart TD
    ...
    Health -->|"missing or incompatible"| Stage["Stage bundled PetCore, CLI, manifest, interaction attestation"]
...
Every managed V3 runtime directory also carries the regular, non-symlink
`interaction-attestation.json` generated by the native Swift interaction
suites for that exact build. PetCore and `petcore-cli` resolve it beside the
versioned executables; published V1 rollback runtimes remain exempt from this
...
the presented center. The Swift and Rust projections canonicalize x/y to the
same exact 1/256 pt grid; visible-screen bounds are quantized inward before
clamping so the canonical center cannot round back outside the hard boundary.
PetCore persists an `external_reposition` or `reset`
...
After checking the expected placement revision, an update whose canonical
placement and intent are unchanged returns success without changing row time,
placement revision, or global state revision. The App's one journal worker
uses a shared five-attempt budget for transport failures and conflicts with
delays of 0, 250, 500, 1000, and 2000 ms. Exhaustion retains the latest journal
and presentation but stops work until bootstrap/reconnect, a newer local
commit, App restart, or explicit retry; ordinary snapshots cannot restart it.
Only the currently pending generation may consume a response.
...
windows as one composition root. Every show path repairs AppKit attachment
before ordering an auxiliary window in. A direct drag freezes auxiliary
relative frames, coalesces parent movement to the target display link, applies
the mouse-up event's exact final frame synchronously, and runs one resting
layout after compare-and-finish finalization. The bubble keeps a legal previous
above/below/left/right anchor when possible and uses a 304–360 pt adaptive
width around its preferred 344 pt. Child-window geometry does not cross RPC.

Overlay interaction diagnostics use one process-local `OSSignpostID` per
gesture and a closed event/result vocabulary. Signposts and opt-in validation
summaries contain only timing/count buckets, attempt 1–5, clamp state, and
auxiliary-visibility booleans; they never contain coordinates, frames,
display/pet/Agent/session identifiers, paths, RPC payloads, or user text.
Release builds add no network telemetry. Setting an absolute
`APC_OVERLAY_PERFORMANCE_SUMMARY_PATH` opts a validation launch into writing
one aggregate-only local JSON summary after convergence.
...
and quarantine table intentionally keep SQLite at released schema 6. Before
current typed projection, PetCore drops persisted Agent rows outside the
seven-event allowlist, normalizes the behavior event map, and moves packages
with removed state names into quarantine without rewriting their assets or
aliasing old actions. The checkpoint still preserves the complete data-level
state needed by those published runtimes.
...
bundle. Retired Studio Skill ownership comes from one structured digest history
shared by PetCore and the release validator. Official release validation keeps
that history append-only and, whenever the Studio Skill changes, requires the
previous release's exact digest while rejecting unrelated additions and the
...
| Pet library | list with validated V3 nine-action timing/playback contracts and derived current revision metadata; bounded typed revision/job history; activate, delete, validate/import/seed/export `.petpack`; forced runtime-asset repair |
...
exact frame inventory, a passing shared transparency report with inspected
multi-background previews, and passing incremental motion QA. Opaque source
rows, source-resolution transparent masters, completed runtime rows, and the canonical
production base stay on disk and are never regenerated merely
because the turn budget ended. A complete source is accepted only after all
nine frame inventories, final motion QA/review, the shared
...
the canonical cover and all nine runtime-action directories, atomically replaces
...
selected V3 action's `frame_durations_ms`, playback mode, settle/cooldown data,
...
without replaying missed frames. Thinking, tool, and done use bounded
`burst_then_idle` presentation so a long semantic lease never freezes on its
last action pose; waiting and failed retain an explicit persistent settle.
App-local acknowledge and left/right drag actions overlay the semantic visual
without emitting Agent events or persisting. There is no gaze-direction or
hover action. Only an idle primary click selects acknowledge. When the bubble has
no content, pointer-down also applies a 160 ms low-amplitude press response;
dragging cancels it. Reduce Motion skips acknowledge and the press response, uses
static drag representatives, and returns finite semantic actions to idle after
their authored active duration.

Allowlisted navigation includes the closed surface value `chatgpt_app`, `claude_app`, `opencode_app`, `cli_terminal`, or `unknown`; the closed `capability` value `exact_session`, `agent_host`, or `unavailable`; and only the target fields needed to prove that capability. A validated Warp URL provides exact CLI routing. A canonical 36-character UUID in the dedicated `routable_session_id` provides exact routing only for the matching ChatGPT/Codex App or Claude App surface. OpenCode App exposes host activation because its audited URL protocol has no existing-session route; a known CLI terminal may likewise provide host-only activation. Pi is CLI-only. Source/surface mismatches, malformed targets, unknown terminal hosts, and historical rows without trustworthy origin metadata fail closed. Explicitly closed successful sessions leave the active projection instead of becoming unavailable rows. The App labels each routable row as App or CLI and names the actual host in host-level action copy without consuming either bounded detail line. Only the concrete session row invokes this navigation path; clicking the pet body merely toggles the session bubble once and cannot open an Agent destination. The App checks the real URL-open, running-App activation, or App-launch outcome. Exact-session rows are consumed only after the exact URL succeeds; opening the same host after a rejected deep link is a degraded recovery action, keeps the row, and publishes a localized retryable navigation error. Host-level rows are consumed only after their declared host opens successfully. Active rows also carry a closed summary kind and opaque animation identity. At most eight concrete sessions are returned, with `active_agent_sessions_omitted_count` representing the bounded remainder. A collapsed Agent group shows one attention-prioritized/latest row; expanding it shows every concrete row for that Agent already present in the bounded snapshot, while the global omitted summary continues to represent sessions beyond the PetCore projection.

Each visible row derives its trailing badge from the stable normalized event, not from the pet action. The closed event vocabulary is `start`, `thinking`, `plan`, `tool`, `waiting`, `done`, and `failed`; `tool` may be refined by the closed summary vocabulary when PetCore has a stable semantic subtype. `start` is request admission and never stands in for thinking. The separate Swift `ProductLifecycleState` mapping leaves `start` at `idle`, maps both `thinking` and `plan` to the package `thinking` action, and maps the remaining four events to their namesake pet states. This same split is used by Pet Configuration event filters and previews, menu-bar recent activity, Pet Library current state, and desktop-bubble badges, so event copy never has to masquerade as animation copy. Persisted protocol titles are validation/audit data and never become a second localized UI authority. Each bubble row's two detail lines render distinct bounded Agent/activity content when available and omit duplicate generic lifecycle copy. The explicit `events.recent` RPC remains the separate stored-event interface and is not reused by the App snapshot.
