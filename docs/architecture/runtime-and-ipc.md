...
A primary cold launch queues one request through the control-center presentation coordinator after AppKit finishes launching. The coordinator is the only owner of desired visibility, App activation, exact-window registration, and the key-window transition. If SwiftUI has not installed the presenter or registered its `NSWindow` yet, the request remains pending and is fulfilled once in either ordering. Cold launch, Dock reopen, secondary-instance activation, product actions, and update recovery therefore converge on the same singleton window without activating the App before that window can be fronted.
...
At bootstrap, ordinary windows are visible and interactive immediately with the system appearance. Persisted appearance hydration is an idempotent `NSAppearance` update only: it never changes window alpha, input handling, ordering, layout, activation, or key state. A stalled PetCore read therefore cannot leave a window blank or transparent. Initial launch and recovery coalesce onto one behavior → seed → snapshot → overlay pipeline. PetCore's first complete snapshot is the final authority for settings, onboarding, pets, placement, connections, and active sessions.
...
Display width is stored separately and changed only through Pet Configuration → Appearance: a 1 pt-step slider with numeric readout, restore-default, keyboard, and VoiceOver adjustment. The range and default live in [Data model](data-model.md#settings-and-first-run); height follows the fixed 208/192 canvas ratio. The overlay has no resize handle, resize hit region, or hover resize control; dragging only moves the pet.

The session bubble attaches only to the pet's top or bottom edge at the authored fixed vertical gap. The side flips when the current one no longer fits; a screen edge changes only horizontal attachment. Vertical distance is never clamped, shortened, or traded for fit, and attachment never moves to the left or right edge. Bubble and menu geometry stays App-local and does not cross RPC. Explicitly closing the Control Center is not classified as the system Show Desktop gesture. Interaction telemetry is aggregate-only and excludes coordinates, display/session identifiers, paths, payloads, and user text.
...
New and resumed Studio turns default to `gpt-6-astra` with `medium` reasoning effort. `APC_PET_STUDIO_CODEX_MODEL` preserves an explicit model override after trimming whitespace; an unset, empty, or longer-than-80-character value uses the default. Model execution and account access belong to the configured Codex App Server. / 新建与恢复的 Studio turn 默认使用 `gpt-6-astra`，推理强度保持 `medium`。`APC_PET_STUDIO_CODEX_MODEL` 去除首尾空白后可显式覆盖模型；未设置、空值或超过 80 个字符时使用默认模型。模型执行与账户访问由配置的 Codex App Server 负责。
