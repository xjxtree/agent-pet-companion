...
The `apc.agent-event.v1` envelope contains allowlisted, bounded fields needed for identity, ordering, activity, navigation, and session display. Explicit bounded session titles plus first/latest user and latest assistant messages are retained for the local bubble. The projected title is the latest explicit session title, or the bounded first user message until one arrives; later user messages update context without silently renaming the session. Arbitrary command/tool payloads, hidden reasoning, complete transcript archives, credential stores, and full process environments are outside the model. External title/detail strings are accepted only for compatibility and are not substituted for the explicit display fields.
