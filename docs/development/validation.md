...
| `simulated integration` | `validate_portable_pet_maker.sh`, `validate_connectors_runtime.sh` | Integration behavior in isolated `APC_HOME`/`APC_AGENT_CONFIG_HOME`, portable pet creation/editing, actual-size Standard/Smooth motion-QA artifacts, synthetic crossfade/interpolation rejection, frame-bound per-state review freshness, generated connector artifacts, event normalization, filtering, and library flows. / 隔离目录中的便携宠物制作、真实尺寸标准/流畅动作 QA、合成叠化/插值拒绝、与帧绑定的逐状态复核、连接器与数据流。 | Artistic quality from a real model, user Agent trust/authentication, real model/tool execution, or visible overlay acceptance. / 不代表真实模型的艺术质量、用户真实 Agent 与可见悬浮层。 |
