...
Authoritative CI never consumes local checkpoints or calls the local fast gate. Portable contracts, source fetch, Rust lint, one linked Rust test build, and the five inventory-checked Rust execution shards run on Ubuntu; a bounded macOS platform shard proves Apple-only clippy and process/lifecycle behavior. Portable pet-maker validation may create a temporary build-ID/contract-digest-bound interaction fixture, but it is never uploaded or accepted as candidate/source proof; those identities require the independent native Swift producer. Cargo source caches have one writer and compiled-target consumers use exact OS, toolchain, lockfile, profile, and target identities without fuzzy restore prefixes. Swift compiles once with strict concurrency and warnings-as-errors into a bounded exact-identity artifact; overlay and App-bundle consumers restore that artifact and run in parallel. Each successful Rust shard uploads a run/attempt-scoped completion marker, and an independent aggregate requires the exact five-marker set. Title/body-only PR edits run scope and `Required CI` bookkeeping without recreating merge tickets; retargeting and source changes remain actionable. / 权威 CI 将可移植合同、依赖获取、Rust lint、单次链接构建与五个执行分片迁至 Ubuntu，并保留有界 macOS 平台分片。可移植宠物制作验证可创建临时且绑定 build ID/合同 digest 的交互 fixture，但绝不上传或充当 candidate/source proof；后两者必须来自独立原生 Swift producer。Cargo source cache 单写，编译缓存使用精确身份且无模糊回退；Swift 以严格并发和零警告单次构建为有界精确产物，悬浮层与 App bundle 并行复用。标题/正文编辑只运行范围与汇总记账，不重建合并票据；改目标分支和源码变更仍是 actionable。

The `Required CI` summary uploads a bounded, content-free engineering metrics artifact. Definitions cover queue/execution/observed-critical-path time, Linux/macOS job-minutes, exact cache observations, aggregate cache restore/save step time, inventory size and 24-hour churn, proof-promotion outcome and closed fallback category, domain/Amber/Red/hotspot counts, claim-overlap and merge-tree preflight rejection counts, and task-to-ready/ready-to-merge timestamps when supplied. Metrics contain no source contents, credentials, or user paths and are compared through CI artifacts rather than durable progress documents. / `Required CI` 汇总会上传有界、无源码内容的工程指标 artifact，覆盖排队/执行/观测关键路径、Linux/macOS job-minutes、精确缓存观察、cache restore/save 聚合耗时、库存体积与 24 小时 churn、证明晋升与封闭回退类别、领域/Amber/Red/热点计数、声明与 merge-tree 拒绝次数，以及可选的任务阶段耗时；指标不含源码内容、凭据或用户路径，也不写入长期进度文档。
