...
| Release / 发布 | source gate once, explicit stress, parallel thin builds, one native packaged acceptance per architecture / 一次源码门禁、显式压力测试、双架构并行构建、每架构一次原生验收 | repeated package execution after trusted digest equality |
