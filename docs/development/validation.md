...
Authoritative CI never consumes local checkpoints or calls the local fast gate. One classifier fans work out into static contracts, integration contracts, Rust lint, five inventory-checked Rust test shards, complete Swift interaction proof, bounded stress, offline overlay proof, and App assembly. Each successful Rust shard uploads a run/attempt-scoped completion marker; an independent aggregate gate requires the exact five-marker set before candidate proof, main source proof, or `Required CI` may succeed, rather than trusting the matrix summary alone. Because markers never cross attempts, recover a failed aggregate with GitHub's **Re-run all jobs**, not **Re-run failed jobs**. Every successful PR `Required CI` emits a short-lived merge ticket bound to repository, PR, base/head refs and SHAs, tested merge parents, run/attempt, and lane. Ready direct or final train PRs to `main` also emit a candidate proof bound to the tested merge commit, exact tree, gate inventory, toolchains, and interaction bytes. Task PRs to `gd-ops/train/*` remain path-scoped, create no Release candidate, and have no duplicate train-push run. / 权威 CI 不消费本地检查点，也不调用本地快速门禁。统一分类结果会并行路由到静态合同、集成合同、Rust lint、5 个带清单校验的 Rust 测试分片、完整 Swift 交互证明、压力、离线悬浮层证明与 App 组装。每个成功 Rust 分片都会上传绑定 run/attempt 的完成标记；独立汇总门禁必须看到精确的 5 个标记集合，候选证明、main 源码证明与 `Required CI` 才能成功，而不会只信任矩阵汇总状态。完成标记不会跨 attempt 复用，因此汇总失败后必须使用 GitHub 的 **Re-run all jobs**，不能使用 **Re-run failed jobs**。每个成功的 PR `Required CI` 都会生成短期 merge ticket，绑定仓库、PR、base/head ref 与 SHA、测试合并父提交、run/attempt 和通道。ready 的 direct 或最终 train → `main` PR 还会生成绑定测试合并 commit、精确 tree、门禁清单、工具链与交互证明字节的候选证明。task → train PR 保持按路径验证、不生成 Release 候选，且合并后不重复运行 train push CI。
