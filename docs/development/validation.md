...
Authoritative CI never consumes local checkpoints or calls the local fast gate. Portable contracts, source fetch, Rust lint, one linked Rust test build, and the five inventory-checked Rust execution shards run on Ubuntu; a bounded macOS platform shard proves Apple-only clippy and process/lifecycle behavior. Cargo source caches have one writer and compiled-target consumers use exact OS, toolchain, lockfile, profile, and target identities without fuzzy restore prefixes. Swift compiles once with strict concurrency and warnings-as-errors into a bounded exact-identity artifact; overlay and App-bundle consumers restore that artifact and run in parallel. Each successful Rust shard uploads a run/attempt-scoped completion marker, and an independent aggregate requires the exact five-marker set. Title/body-only PR edits run scope and `Required CI` bookkeeping without recreating merge tickets; retargeting and source changes remain actionable. / 权威 CI 将可移植合同、依赖获取、Rust lint、单次链接构建与五个执行分片迁至 Ubuntu，并保留有界 macOS 平台分片。Cargo source cache 单写，编译缓存使用精确身份且无模糊回退；Swift 以严格并发和零警告单次构建为有界精确产物，悬浮层与 App bundle 并行复用。标题/正文编辑只运行范围与汇总记账，不重建合并票据；改目标分支和源码变更仍是 actionable。
