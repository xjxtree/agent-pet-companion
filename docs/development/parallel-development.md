...
## Codex project defaults / Codex 项目默认设置

The repository's [`.codex/config.toml`](../../.codex/config.toml) selects `gpt-6-astra` for Codex development sessions. Codex loads this project layer only for trusted projects; explicit session/CLI model choices and managed host requirements still apply. The repository leaves reasoning effort to the contributor's configuration. Keep a compatible effort when migrating; an inherited `none` or `minimal` setting needs `low` or another supported effort. / 仓库项目配置为 Codex 开发会话选择 `gpt-6-astra`。Codex 仅在项目受信任时加载此层，显式会话或命令行模型选择及宿主管理要求仍然适用。推理强度沿用贡献者配置；迁移时保留兼容强度，若继承的是 `none` 或 `minimal`，需改用 `low` 或其他受支持强度。

[AGENTS.md](../../AGENTS.md) retains repository-wide boundaries and routes each task to its owning contract; it does not duplicate host-level execution, clarification, or communication instructions. [CONTRIBUTING.md](../../CONTRIBUTING.md) is the contributor entrypoint. `.codex/environments/environment.toml` is generated by the Codex App and owns its local Run action; model selection belongs in `config.toml`. / AGENTS.md 保留仓库通用边界并按任务指向对应契约，不重复宿主层的执行、澄清或沟通指令；CONTRIBUTING.md 是贡献者入口。Codex App 生成的 environment.toml 负责本地 Run 操作，模型选择由 config.toml 管理。

The in-App AI Pet Maker has a separate PetCore-owned `gpt-6-astra` default with `medium` reasoning and the bounded `APC_PET_STUDIO_CODEX_MODEL` override described in [Runtime and IPC](../architecture/runtime-and-ipc.md). Project configuration does not select the image-generation provider or change the portable Maker/Studio production contracts. / App 内 AI 宠物制作由 PetCore 独立设置 `gpt-6-astra`、`medium` 推理强度及有界模型覆盖。项目配置不选择生图供应商，也不改变便携 Maker 或 Studio 的制作契约。

These defaults follow the [official Astra migration guidance](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-6-astra) and [Codex configuration reference](https://learn.chatgpt.com/zh-Hans/docs/config-file/config-reference). / 默认设置依据上述官方迁移与配置文档。
...
Fork PRs and PRs from ordinary branches targeting `main` use the same required validation as managed development PRs. Merge-ticket eligibility is separate: only a nonempty matching source-repository identity and a managed `gd-ops/task/*`, `gd-ops/fix/*`, `gd-ops/train/*`, or `gd-ops/release/*` branch may create and upload a ticket. Forks, unmanaged branches, and missing repository identities skip those two steps and require a maintainer's manual squash merge after the required checks pass. Renaming a contributor's branch is unnecessary. / fork PR 和以普通分支为来源、目标为 `main` 的 PR 与受管开发 PR 执行相同的必需验证。合并票据资格单独判断：只有来源仓库身份非空且与本仓库一致，同时分支属于受管的 `gd-ops/task/*`、`gd-ops/fix/*`、`gd-ops/train/*` 或 `gd-ops/release/*`，才可生成并上传票据。fork、非受管分支及缺少仓库身份的 PR 跳过这两个步骤，在必需检查通过后由维护者手动 squash 合并，无需改名贡献者的分支。
