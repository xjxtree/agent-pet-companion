...
[AGENTS.md](../../AGENTS.md) owns task execution, clarification, communication, and proportional validation guidance. [CONTRIBUTING.md](../../CONTRIBUTING.md) is the contributor entrypoint. `.codex/environments/environment.toml` is generated by the Codex App and owns its local Run action; model selection belongs in `config.toml`. / AGENTS.md 负责执行、澄清、沟通与适量验证约定；CONTRIBUTING.md 是贡献者入口。Codex App 生成的 environment.toml 负责本地 Run 操作，模型选择由 config.toml 管理。
