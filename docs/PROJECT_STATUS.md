...
| 原生 UI | 控制中心、状态栏、桌宠控制、右击菜单和消息气泡使用 macOS 原生 clear Liquid Glass。消息气泡在 macOS 26 使用无 tint/实色填充的 `NSGlassEffectView.Style.clear` 背景 sibling；“启用与行为”提供 0–100% 透明度实时调节，默认 55%，内部映射为 0.88–0.30 原生光学强度，最高透明仍保留液态折射。文字、图标和控件位于独立、100% 不透明的 SwiftUI sibling 前景层，不使用模糊、白色/黑色 halo 或文字阴影；辅助功能优先。macOS 14–15 使用系统 `ultraThinMaterial` 回退。 |
...
| Swift 单元测试 | 引入固定 `swift-testing` 依赖与兼容 runner；当前 103 tests / 11 suites 通过。 |
...
| Swift tests | PASS，103 tests / 11 suites（含原生 clear glass 背景/前景 sibling 层级、可调光学强度端点、最终像素合成、完整尺寸/远端命中、生成历史空响应兼容、玻璃辅助功能回退、同步 RPC envelope 解码与旧 `apc.runtime-manifest.v1` 新增字段重建回归）；`swift-testing` 固定为兼容 CI Swift 6.1.2 的 6.1.3，直接 `swift test` 与本地兼容验证入口均通过。 |
| Rust workspace tests、fmt、clippy `-D warnings` | PASS：258/258，0 failure |
| Swift 全量测试 | PASS：103/103，0 failure |
...
- 当前 develop build：`0.1.0.1.20260716133534.40626`；App、PetCore 与 CLI 已在真实版本交接后确认一致。
- ZIP SHA-256：`118089edc9c76d27e8306c132d7cf9b319d41d5d4f563d5b232d510c6df0aa6a`。
