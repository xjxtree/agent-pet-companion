...
| 原生 UI | 控制中心、状态栏、桌宠控制、右击菜单和消息气泡使用 macOS 原生 clear Liquid Glass。消息气泡在 macOS 26 使用无 tint、无填充、无边框、无附加 opacity 的最高透明 clear interactive glass，并把前景直接置于玻璃内容层；macOS 14–15 使用系统 `ultraThinMaterial` 回退。 |
...
| Swift tests | PASS，96 tests / 10 suites（含玻璃辅助功能回退与旧 `apc.runtime-manifest.v1` 新增字段重建回归） |
...
| 最大透明气泡回归 | PASS：Computer Use 完成旧 UI Host 退出与新 bundle 启动，运行进程晚于新构建且 build ID 为 `0.1.0.1.20260716031517.53137`；静默窗口像素检查确认标题、当前活动和状态均可见，气泡后方浅色界面文字/分隔线仍可辨认。正常态无 tint、填充、边框或玻璃 opacity；Reduce Transparency/Increase Contrast 使用深色可读性回退。 |
...
- 当前 develop build：`0.1.0.1.20260716031517.53137`。
- ZIP SHA-256：`0d67cb080debd39e1725916cc1b687ba30e3a645a56c51c65dff4b0a3fb46739`。
