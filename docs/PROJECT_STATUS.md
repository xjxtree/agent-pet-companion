...
| 原生 UI | 控制中心、状态栏、桌宠控制、右击菜单和消息气泡使用 macOS 原生 clear Liquid Glass。消息气泡在 macOS 26 使用无 tint/实色填充的 `NSGlassEffectView.Style.clear` 背景 sibling；“启用与行为”提供 0–100% 透明度实时调节，默认 55%，内部映射为 0.88–0.30 原生光学强度，最高透明仍保留液态折射。文字、图标和控件位于独立、100% 不透明的 SwiftUI sibling 前景层；辅助功能优先。macOS 14–15 使用系统 `ultraThinMaterial` 回退。 |
