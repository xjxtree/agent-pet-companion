...
| 原生 UI | 控制中心、状态栏、桌宠控制、右击菜单和消息气泡使用 macOS 原生 clear Liquid Glass。“启用与行为”提供跟随系统/黑色/白色三种全局主题，统一驱动 AppKit appearance、SwiftUI 根视图、四个桌宠 Panel 和原生菜单；旧配置默认跟随系统。消息气泡透明度继续独立实时调节，默认 55%，内部映射为 0.88–0.30 原生光学强度，切换主题不改透明度。文字、图标和控件位于独立、100% 不透明的 SwiftUI sibling 前景层，不使用模糊、白色/黑色 halo 或文字阴影；辅助功能优先。macOS 14–15 使用系统 `ultraThinMaterial` 回退。 |
...
| 生成会话历史 | App 内生成宠物可从宠物库恢复私有生成消息；`generation.for_pet` 的无记录响应保持稳定空结构，App 同时兼容旧 PetCore 缺少 `messages` 的响应。外部导入包不携带私有会话，界面明确显示“无 App 内生成会话”，仍可发起新的 AI 修改。 |
| 宠物库可移植性 | 任意合规 `.petpack` 进入同一校验/不可变 revision 链；支持原子导出、重新导入、保持活跃状态的同 ID 修订，以及对 App 创建或外部导入宠物发起 Codex AI 修改。 |
| 外部 Agent 制作 | 新增 provider-neutral `agent-pet-maker` 技能；Codex、Claude Code、Pi、Hermes、OpenCode 等具备真实图像能力的宿主可创建/修改、校验与构建包。缺能力时返回 `capability_missing`；仅在用户明确授权时通过在线 daemon 导入/激活。Codex 已完成真实 `image_gen.imagegen` 创建、导入、激活、七状态桌面使用和重启保持的完整验证。 |
...
| Swift 单元测试 | 引入固定 `swift-testing` 依赖与兼容 runner；当前 105 tests / 11 suites 通过。 |
...
| Swift tests | PASS，105 tests / 11 suites（含三态原生主题映射、主题/透明度正交性、原生 clear glass 背景/前景 sibling 层级、可调光学强度端点、最终像素合成、完整尺寸/远端命中、生成历史空响应兼容、玻璃辅助功能回退、同步 RPC envelope 解码与旧 `apc.runtime-manifest.v1` 新增字段重建回归）；`swift-testing` 固定为兼容 CI Swift 6.1.2 的 6.1.3，直接 `swift test` 与本地兼容验证入口均通过。 |
| Rust workspace tests、fmt、clippy `-D warnings` | PASS：261/261，0 failure |
| Swift 全量测试 | PASS：105/105，0 failure |
...
| 三态主题与气泡透明度兼容 | PASS：Computer Use 逐项切换“跟随系统 / 黑色主题 / 白色主题”，控制中心即时换色且 PetCore 持久化成功；每次切换气泡透明度均保持 25%。黑色主题下关闭主窗口后，以不移动鼠标、不切换焦点的只读系统截图确认消息气泡、收起按钮与缩放按钮继续采用透明原生玻璃，并按深色语义显示。右击菜单与四个 overlay panel 使用相同 `NSAppearance`，Swift 回归覆盖三种主题映射以及透明度光学强度不变。最终已恢复“跟随系统”。证据：`dist/evidence/theme-validation/dark-overlay.png`。 |
...
| 最大透明气泡回归 | PASS：修复了把完整 `NSHostingView` 放入 `NSGlassEffectView.contentView` 后真实合成只剩玻璃、整棵前景消失的回归；新增离屏像素测试会直接比较 foreground/empty 的最终合成，不再只检查 ownership/frame。当前实现为无 tint/填充/边框的原生 `NSGlassEffectView.Style.clear` 背景 sibling，用户透明度将原生光学层强度从 0.88 线性调节到 0.30，文字/图标/控件为独立 100% opacity sibling。Computer Use 完成旧 UI Host 退出与新 bundle 启动，App/PetCore/CLI build ID 一致为 `0.1.0.1.20260717042613.13680`；由于透明 panel 的 AX 树只返回缩放控制，随后用不移动鼠标、不切换焦点的只读系统截图在深浅分割背景确认文字可读且后方内容明显可见。Reduce Transparency 使用不透明外观匹配回退，Increase Contrast 使用增强对比回退。证据：`dist/evidence/liquid-glass-max-clear-v2/full-screen.png`、`bubble-detail.png`。 |
...
| 真实 Codex `agent-pet-maker` create→install→activate | PASS：`Bytebud 字节芽`（`pet_bytebudcodex`）由 `image_gen.imagegen` 生成 384×416 RGBA、7 状态 × 2 帧，validation `ok=true`、14 frames、0 warning；真实导入/激活、App 宠物库、七状态桌面动作和 Quit→重新打开保持全部确认。Fresh Codex CLI 0.144.4 验证插件限定名 `$agent-pet-companion:agent-pet-maker` 会可靠注入正文；短名仅解析、不注入，文档与默认提示已改用限定名。 |
| Portable Skill 安装与视觉加固回归 | PASS：覆盖原子 `expect-absent`、提交后连接错误的精确 archive SHA 对账且不二次导入、导入性能/TOCTOU、严格透明与可见覆盖、状态内真实帧差异、cover 可见性、动态预览完整解码与帧数/解码内存预算，以及放宽后的 App Server probe/真实测试 deadline。 |

### 7.1 真实 Codex Agent Pet Maker 端到端证据

- 制作身份：`Bytebud 字节芽`，ID `pet_bytebudcodex`；Codex 通过 `agent-pet-maker` 调用真实 `image_gen.imagegen`，包内 `source/source.json` 记录 `runner=OpenAI Codex`、`provenance=skill-full-source`，不是确定性占位资源。
- 视觉与结构：384×416 RGBA；`idle`、`start`/thinking、`tool`、`waiting`、`review`、`done`、`failed` 七个固定状态各 2 帧，共 14 帧；PetCore validation 为 `ok=true`、0 warning。
- 产物：`dist/e2e/agent-pet-maker-codex/bytebud-codex.petpack`；SHA-256 `fa1754d815d8aa544e254880183c7ca920098becb32c8e612e4b585d58ed74e0`。同目录保留结果 JSON、contact sheet 和实机截图。
- 安装语义：首次在线导入请求在 PetCore 已提交后遇到 Unix socket read timeout，因此原始 `bytebud-codex.install-result.json` 保留 `partial_success` 取证，而不伪装成一次成功响应；随后以 PetCore 列表/快照、已安装不可变 archive SHA 和 App 宠物库权威回读确认真实导入与激活。该场景推动了“歧义失败只对账、不盲目重试”、原子 `expect-absent` 和导入性能修复。
- 桌面使用：`runtime-idle.png`、`runtime-thinking.png`、`runtime-tool.png`、`runtime-waiting.png`、`runtime-review.png`、`runtime-done.png`、`runtime-failed.png` 逐一记录七状态真实渲染；`runtime-waiting-delayed.png` 与 `runtime-review-delayed.png` 补充延迟状态转换证据。
- 生命周期：App 真正 Quit 后重新打开，`runtime-after-restart.png` 确认 `Bytebud 字节芽` 仍为已启用桌宠，导入和激活状态持久化。
...
- 当前 develop build：`0.1.0.1.20260717042613.13680`；App、PetCore 与 CLI 已在真实版本交接后确认一致。
- ZIP SHA-256：`e117a2dd757dc6bc4749f22d5069d2a35451f0c5aac2c5b2ea7203714d5e8d5f`。
