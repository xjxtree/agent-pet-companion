...
| Swift 单元测试 | 引入固定 `swift-testing` 依赖与兼容 runner；当前 101 tests / 11 suites 通过。 |
...
| Swift tests | PASS，101 tests / 11 suites（含原生 clear glass 背景/前景 sibling 层级、最终像素合成、完整尺寸/远端命中、玻璃辅助功能回退、同步 RPC envelope 解码与旧 `apc.runtime-manifest.v1` 新增字段重建回归）；`swift-testing` 固定为兼容 CI Swift 6.1.2 的 6.1.3，直接 `swift test` 与本地兼容验证入口均通过。 |
| Rust workspace tests、fmt、clippy `-D warnings` | PASS：258/258，0 failure |
| Swift 全量测试 | PASS：101/101，0 failure |
