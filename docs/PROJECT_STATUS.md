...
| 原生 UI | 控制中心、状态栏、桌宠控制、右击菜单和消息气泡使用 macOS 原生 clear Liquid Glass。消息气泡在 macOS 26 使用无 tint/填充/边框的 `NSGlassEffectView.Style.clear` 背景 sibling，最大透明基线仅保留 18% 光学强度；文字、图标和控件位于独立、100% 不透明的 SwiftUI sibling 前景层。macOS 14–15 使用系统 `ultraThinMaterial` 回退。 |
...
| Swift 单元测试 | 引入固定 `swift-testing` 依赖与兼容 runner；当前 102 tests / 11 suites 通过。 |
...
| Swift tests | PASS，102 tests / 11 suites（含原生 clear glass 背景/前景 sibling 层级、最终像素合成、完整尺寸/远端命中、生成历史空响应兼容、玻璃辅助功能回退、同步 RPC envelope 解码与旧 `apc.runtime-manifest.v1` 新增字段重建回归）；`swift-testing` 固定为兼容 CI Swift 6.1.2 的 6.1.3，直接 `swift test` 与本地兼容验证入口均通过。 |
| Rust workspace tests、fmt、clippy `-D warnings` | PASS：258/258，0 failure |
| Swift 全量测试 | PASS：102/102，0 failure |
