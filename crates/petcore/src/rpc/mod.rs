...
use crate::generation as generation_service;
use crate::metrics;
...
impl CoreState {
    ...
    pub fn ensure_ready(&self) -> Result<()> {
        ...
        let result = (|| {
            ...
            if let Err(error) = generation_service::recover_interrupted_jobs_for_instance(
                &self.paths,
                &self.database,
                &self.instance_id,
            ) {
...
fn handle_core_request(state: &CoreState, request: RpcRequest) -> Result<Value> {
...
fn active_generation_snapshot(state: &CoreState) -> Result<Option<Value>> {
    ...
    let recovery_form = generation_service::generation_recovery_form(&state.paths, &job)?;
    let operation = generation_service::generation_job_operation(&job);
    let baseline_revision_id =
        generation_service::generation_job_baseline_revision_id(&state.paths, &job)?;
    let messages =
        generation_service::read_messages_with_database(&state.paths, &state.database, &job.id)?;
    ...
    let capabilities = generation_service::generation_job_capabilities(&job, false);
    Ok(Some(json!({
        "job_id": job.id,
...
fn generation_session_recovery_snapshot(
    ...
    let baseline_revision_id =
        generation_service::generation_job_baseline_revision_id(&state.paths, job)?;
    let result =
        generation_service::read_generation_result(&state.paths, &state.database, &job.id)?;
    let cancellation_pending =
        job.cancel_requested_at.is_some() && job.thread_archived_at.is_none();
    let capabilities = generation_service::generation_job_capabilities(job, false);
    Ok(json!({
        ...
        "reference_reselection_count": recovery_form.reference_reselection_count,
        "message_revision": state.database.generation_message_revision(&job.id)?.to_string(),
        "messages": generation_service::read_messages_with_database(
            &state.paths,
            &state.database,
            &job.id
        )?
