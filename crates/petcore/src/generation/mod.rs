            ...
            match prepare_and_import_skill_petpack_source(
                ...
                SkillPetpackImport::Imported { pet, previous_pet } => {
                    complete_imported_pet(
                        ...
                        messages::COMPLETION_LIBRARY_READY_TAIL,
                    )?;
                    return Ok(());
                }
...
fn run_reply_revision(
        ...
        != Some(true)
    {
        ...
        let detail = app_server_session
            ...
            .and_then(serde_json::Value::as_str)
            .unwrap_or(messages::APP_SERVER_UNAVAILABLE_DETAIL);
        fail_generation(
            paths,
            database,
            job_id,
...
fn ensure_timing_changed_frames(
    ...
    let result = changed_states.iter().try_for_each(|state| {
        ...
        let base_frames = visual_qa::decoded_state_frame_digests(&base_dir.join(&relative))?;
        let output_frames = visual_qa::decoded_state_frame_digests(&output_dir.join(&relative))?;
...
fn recoverable_checkpoint_pause_message(detail: &str) -> Option<String> {
    match crate::app_server::ExternalCheckpointPauseKind::from_error_detail(detail) {
        Some(crate::app_server::ExternalCheckpointPauseKind::StalledCheckpointTurns { .. }) => {
            Some(
                "制作已暂停：连续 3 个续接段没有检测到新的持久化产物。Codex App Server 连接本身正常；原 job、Codex 会话、素材和已通过 QA 均已保留，可点击“继续制作”从当前工作区接着运行。"
                    .to_string(),
            )
        }
        Some(crate::app_server::ExternalCheckpointPauseKind::WallWindowExceeded) => Some(
            "制作已达到 12 小时连续运行安全窗口并暂停。Codex App Server 连接本身正常；原 job、Codex 会话、素材和已通过 QA 均已保留，可点击“继续制作”接着运行。"
                .to_string(),
        ),
        // Not produced by the current build; retained for historical job
        // records written before pause kinds were typed.
        None if detail.contains("bounded checkpoint turns") => Some(
...
fn derive_pet_name(form: &GenerationForm, ai_brief: Option<&serde_json::Value>) -> String {
    ...
    if trimmed.is_empty() {
        return messages::DEFAULT_PET_DISPLAY_NAME.to_string();
    }
    ...
    let name: String = first_phrase
        ...
        .collect();
    if name.is_empty() {
        messages::DEFAULT_PET_DISPLAY_NAME.to_string()
    } else {
        name
    }
}
