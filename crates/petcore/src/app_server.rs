...
6. Resolve the sibling agent-pet-maker skill and read both `references/visual-production-and-native-resolution.md` and `references/transparent-frame-production.md`. Use built-in imagegen by default and request genuine transparent PNG source art: Alpha 0 exterior and pose gutters, authored antialiasing, and no checkerboard, backdrop, floor, shadow, glow, text, or watermark. Copy each selected original from `$CODEX_HOME/generated_images` into this private job workspace while leaving the built-in original in place. Persist and inspect decoded RGBA, require exact frame count/order, complete stable action poses, and subject scale matching the size reference, then crop stable equal-size 12:13 windows in source pixels. Every crop must be at least the selected target. Use only the version 2 `native_alpha` path in `scripts/prepare_transparent_frames.py` for Alpha preservation, hidden-transparent-RGB normalization, the sole optional downscale, and multi-background QA. An explicitly selected opaque-only source uses its `chroma_key` compatibility mode. Never trust a prompted output size as evidence. A state may proceed only when its report and every frame say ok=true, its previews pass inspection, and the exact-tier runtime animation retains identity, distinct poses, readable action, anatomy, props, continuity, crop, and settle/loop quality. Do not rematte, contract, feather, or silently switch a failing native-Alpha action to chroma; regenerate only that action. `visible_key_pixels` applies only to chroma compatibility input and remains preview-review evidence rather than a nonzero failure.
...
Read the sibling Maker `references/visual-production-and-native-resolution.md` and `references/transparent-frame-production.md`. Lock a canonical identity and author each state as one coherent sequence with an intended whole-character trajectory. For every multi-frame action, generate a deterministic pose guide and a separate deterministic size-reference image from one shared slot, centered 12:13 crop, safe-box, baseline, and global-scale record; pass them after the character base in that order. Generate one state at a time with built-in imagegen by default. Require a genuinely transparent PNG canvas with Alpha 0 exterior and gutters and no checkerboard, backdrop, floor, shadow, glow, text, or watermark; copy the selected original from `$CODEX_HOME/generated_images` into this private workspace while leaving the built-in original in place. Persist and inspect decoded RGBA, require exact frame count/order and subject scale matching the size reference, then crop stable equal-size 12:13 source windows at least as large as the selected target. Use only the version 2 `native_alpha` path in `scripts/prepare_transparent_frames.py` for authored-Alpha preservation, hidden-transparent-RGB normalization, transparent masters, the sole optional downscale, runtime PNGs, and multi-background QA. An explicitly selected opaque-only provider uses the script's `chroma_key` compatibility mode. Do not rely on text-only equal-scale control or the model returning exact target dimensions, independently fit or recenter source poses, batch unrelated states, or synthesize filler by resampling, crossfade, morph, optical flow, transformed duplicates, or procedural interpolation. A reviewed post-transparency `motion-align` pass is the only registration exception and may translate whole frames by integer pixels only.
...
Accept a state only after its transparency report and every frame say ok=true and its checkerboard/white/gray/black/contrast-color preview passes at 100%. Native input must preserve its complete Alpha plane and nontransparent RGBA, retain no hidden RGB beneath Alpha 0, and remain free of canvas contact or baked backgrounds. Treat chroma-only `visible_key_pixels` as diagnostic review evidence rather than a zero-tolerance failure; actual edge fringe or visible contamination remains a blocker. Inspect the exact-tier runtime animation after any downscale with the same identity, distinct-pose, action, anatomy, prop, crop, continuity, and settle/loop requirements as an exact-size source. Keep raw sources, masks, transparent masters, reports, and previews outside `petpack-source`; never replace the shared script with custom thresholds, color removal, Alpha filtering, or resizing.
...
mod timing_normalization_tests {
    ...
    fn external_helper_contract_returns_complete_explicit_timing() {
        ...
        assert!(prompt.contains("built-in imagegen by default"));
        assert!(prompt.contains("genuinely transparent PNG canvas"));
        assert!(prompt.contains("$CODEX_HOME/generated_images"));
        assert!(prompt.contains("version 2 `native_alpha` path"));
        assert!(prompt.contains("`chroma_key` compatibility mode"));
        ...
        assert!(prompt.contains("checkerboard/white/gray/black/contrast-color"));
