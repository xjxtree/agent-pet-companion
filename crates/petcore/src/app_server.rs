...
const STUDIO_GENERATION_EVIDENCE_INSTRUCTIONS: &str = "For Codex imagegen, request native transparent RGBA first for each production base/action. Use the sibling Maker scripts/generation_evidence.py check before generation and record after every actual executed image call, retaining the exact prompt and untouched output in generation-evidence.json plus .generation-inputs outside petpack-source. Before flat_chroma fallback, require at least 3 rejected native_alpha calls for that same object in the current attempt cycle; change the prompt based on each failure. Stop early on an accepted native result. Never count reprocessing, another action, an unexecuted call, or a refusal as an attempt. Resume by validating the existing ledger; never reset it to bypass retries. Use prepare_transparent_frames.py with explicit source_mode: native_alpha preserves source RGBA masters and supports quality-preserving proportional scaling and canvas placement, with no matting, RGB repair, contraction, or feathering; flat_chroma uses the bounded chroma path. Final motion-qa binds the generation ledger digest, and production verification rechecks retained prompts, raw outputs, retry policy, and latest accepted outcomes. For edits, add --baseline base-petpack-source to final combined motion-qa, production-verify, and validate-source; never use --state for final combined QA. Unchanged baseline timing stays intact; the 1000–3200 ms production duration rule applies only to regenerated semantic actions. Keep build/validation.json false during authoring. After reviewed QA, run petpack_workspace.py validate-source --source petpack-source --report motion-qa/report.json --review motion-review.json (with --baseline for edits). This helper verifies readiness, stages the required format marker, invokes the real PetCore CLI, and resets ok:false on failure. Do not manually set the marker to claim validation.";
...
{STUDIO_GENERATION_EVIDENCE_INSTRUCTIONS}"#,
...
For external_full_source, author distinct frames, run incremental and combined motion QA/review plus production-verify, inspect the combined 8–12 second all-action-bound presence preview, reject premature static or mechanical looping without retiming, complete the shared `validate-source` helper, then return:
...
Read the sibling Maker `references/visual-production-and-native-resolution.md` and `references/transparent-frame-production.md`. Lock a canonical identity and author each state as one coherent sequence with an intended whole-character trajectory. For every multi-frame action, generate a deterministic pose guide and a separate deterministic size-reference image from one shared slot, centered 12:13 crop, safe-box, baseline, and global-scale record; pass them after the character base in that order. Generate one state at a time with native transparent RGBA first under the generation-evidence policy below. Never rely on text-only equal-scale control or on the model returning exact target dimensions. Persist and inspect the untouched output, require exact frame count/order and complete distinct poses, then extract source crops without prescribing their dimensions or aspect ratio. Use `scripts/prepare_transparent_frames.py` for transparent masters, quality-preserving proportional scaling, placement on the exact runtime canvas, and multi-background QA. Agents may correct per-frame size and position drift and add transparent margins when anatomy, detail, intended motion, and continuity remain intact. Preserve originals and record transforms; judge final quality before regenerating. Do not batch unrelated states or synthesize missing poses with crossfade, morph, optical flow, transformed duplicates, or procedural interpolation. The optional `motion-align` helper remains integer-translation-only; scaling and framing use the transparency script placement controls.
...
Do not restart the pet, replace the canonical production base, or regenerate any state that already has its exact frame count, matching deterministic pose/size references, a passing shared transparency report with inspected previews, and a passing incremental `motion-qa-<state>/report.json`. Inspect the existing manifest, state directories, guide geometry sidecars, pose guides, size-reference images, untouched native or chroma source rows, transparent masters, transparency reports/previews, and motion-QA artifacts first. Resume with exactly the earliest incomplete or failing state; preserve or regenerate both structural references together from their shared geometry, persist the untouched output, crop complete poses, use quality-preserving proportional scaling and placement in `scripts/prepare_transparent_frames.py`, and pass that state's transparency and runtime-size motion gates before any later state. Compare Motion QA's body-anchor and baseline path with the action card and pose guide. Preserve intentional travel and easing; if registration alone is wrong, try a fresh QA-digest-bound `motion-align` integer-translation pass on transparent frames before regeneration, inspect it, copy only approved PNGs, and rerun Motion QA. Do not require exact model dimensions, source aspect ratio, or guide occupancy. Correct scale and framing with the shared script from retained originals, preserve intended movement, and inspect final detail and continuity. Reassess the latest retained output with generation_evidence.py reassess after processing; this is not a new image call and requires fresh QA/review. Treat `visible_key_pixels` as preview-review evidence, not an automatic nonzero failure. Keep generation serial in this owning turn and do not spawn task workers.
...
mod timing_normalization_tests {
    ...
    fn external_helper_contract_returns_complete_explicit_timing() {
        ...
        assert!(prompt.contains("native transparent RGBA first"));
        assert!(prompt.contains("at least 3 rejected native_alpha calls"));
        assert!(prompt.contains("generation-evidence.json"));
        assert!(prompt.contains("validate-source"));
        assert!(!prompt.contains("never request model-native transparency"));
        assert!(prompt.contains("model returning exact target dimensions"));
        assert!(prompt.contains("proportional scaling"));
    ...
    #[test]
    fn every_studio_entry_and_continuation_keeps_native_retry_and_edit_contracts() {
        ...
        for prompt in [
            pet_studio_developer_instructions("test-job", &form),
            pet_studio_external_helper_prompt(false),
            pet_studio_checkpoint_prompt(2),
            pet_studio_resume_prompt(None),
        ] {
            assert!(prompt.contains(STUDIO_GENERATION_EVIDENCE_INSTRUCTIONS));
            assert!(!prompt.contains("never request model-native transparency"));
            assert!(prompt.contains("--baseline base-petpack-source"));
            assert!(prompt.contains("validate-source"));
        }
    }
