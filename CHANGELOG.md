...
- Fixed narrow native Inspectors so long pet metadata, protocol-state lists, frame-rate profiles, and revision IDs wrap within the available width instead of being clipped. / 修复原生窄检查器中的长宠物元数据、协议状态列表、帧率档位与 revision ID，使其在可用宽度内换行而不再被裁切。
