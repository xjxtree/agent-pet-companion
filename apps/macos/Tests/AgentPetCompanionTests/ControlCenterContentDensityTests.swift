...
struct ControlCenterContentDensityTests {
    ...
    func topLevelPagesUseToolbarActionsWithoutRepeatingNavigationTitles() throws {
        ...
        #expect(!connections.contains("technicalDetailsExpanded"))
        #expect(!connections.contains("ConnectionTechnicalRow"))
        #expect(connections.contains("AgentManagedComponentRow"))
        #expect(connections.contains("\"connections.managed-components."))
        #expect(connections.contains("private var moreActionsMenu"))
        #expect(connections.contains("\"connections.secondary.more."))
        #expect(connections.contains("APCLocalization.text(.appActionMore)"))
        #expect(!connections.contains("private var primaryActionButtons"))
        #expect(!connections.contains("private func managedActionButtons"))
        #expect(!connections.contains("AdvancedDetailsDisclosure("))
        #expect(!connections.contains("connections.extension-inventory"))
        #expect(!connections.contains("AgentExtensionInventory"))
        ...
        #expect(diagnostics.contains("serviceStatusDetails"))
        #expect(diagnostics.contains("ForEach(presentation.rows)"))
        #expect(diagnostics.contains("\"diagnostics.service-details\""))
        #expect(!diagnostics.contains("technicalDetailsRegion"))
        #expect(!diagnostics.contains("isTechnicalDetailsExpanded"))
        #expect(!diagnostics.contains("\"diagnostics.technical-details\""))
